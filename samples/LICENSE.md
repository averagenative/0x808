# Sample License

Samples in this directory come from a mix of public-domain / royalty-free
recordings (the original drum-machine pack) and externally-sourced kits
that require attribution under their respective licenses (Pearl,
Cassette).

## Royalty-free, no attribution required

| Directory     | Source                         |
|---------------|--------------------------------|
| `808/`        | Roland TR-808 Rhythm Composer  |
| `909/`        | Roland TR-909 Rhythm Composer  |
| `505/`        | Roland TR-505 Rhythm Composer  |
| `kicks/`      | Assorted kick drum samples     |
| `snares/`     | Assorted snare drum samples    |
| `hihats/`     | Assorted hi-hat samples        |
| `percussion/` | Assorted percussion one-shots  |
| `mrk2/`       | Assorted drum kit samples      |
| `808-synth/`  | 808-style synthesized one-shots |

## Externally-sourced kits

### `cr-78-real/` — Roland CompuRythm CR-78 (Public Domain)

- License: **Public Domain** (no attribution required)
- Source: Oramics `sampled` repo `DM/CR-78/`, originally from
  http://www.boxedear.com/free.html
- URL: https://oramics.github.io/sampled/DM/CR-78/
- Used by the "CR-78" kit.

### `linndrum/` — LinnDrum LM-2 (Public Domain)

- License: **Public Domain** (no attribution required)
- Source: Oramics `sampled` repo `DM/LM-2/`, originally from
  http://machines.hyperreal.org/manufacturers/Linn/LinnDrum/
- URL: https://oramics.github.io/sampled/DM/LM-2/
- Used by the "LinnDrum" kit.

## Attribution-required kits

### `pearl/` — Pearl Master Studio Pack 1

- Author: **enoe**
- License: **Creative Commons Attribution 3.0 (CC-BY 3.0)**
- Source: https://oramics.github.io/sampled/DRUMS/pearl-master-studio/
- Used by the "Acoustic" kit.

### `trap-808/` — Mailbox Badger PD Drum Samples Vol 1 & 2

- Author: **Mailbox Badger**
- License: Vol 1 = **CC Public Domain Mark 1.0**, Vol 2 = **CC-BY 3.0**
- Sources:
  - https://archive.org/details/HeatDish (Vol 1)
  - https://archive.org/details/mailboxbadgerdrumsamplesvolume2 (Vol 2)
- Used by the "Sub 808" kit.

### `cassette/` — Beckstrom's Weird TR-808 Sample Pack

- Author: **Chris Beckstrom**
- License: **Creative Commons Attribution 4.0 (CC-BY 4.0)**
- Source: https://archive.org/details/808_variations
- Used by the "Cassette" kit. Files transcoded from 32-bit stereo to
  16-bit mono to match the project sample format; original character
  preserved.

## Disclaimer

These samples are not affiliated with, endorsed by, or licensed from
Roland Corporation. They are recordings of sounds produced by hardware
synthesizers and drum machines (or modeled after them), provided as-is
with no warranty.
